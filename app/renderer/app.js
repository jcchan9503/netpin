...
const label = { responding: '本次有响应', mapped: '发现映射', historical: '历史 / 静态线索', unseen: '未发现使用证据', unknown: '待检测 / 无法确认', excluded: '已排除', assigned: '已分配', reserved: '已预留', unassigned: '未分配', unregistered: '未登记', ok: '近期采集正常', partial: '部分可用 / 已过期', never: '尚未采集', up: 'Up', down: 'Down', unknownPort: '状态未知', success: '完成', failed: '失败', cancelled: '已取消', interrupted: '已中断', running: '运行中' };
...
let state, view = 'overview', selectedNet = '', selectedDevice = '', page = 0, query = '', layout = 'grid', searchQuery = '', currentRows = [], renderId = 0, toastTimer, lastFreshnessRender = Date.now();
...
async function devicePage() {
  ...
  html += `<div class="table-wrap"><table><thead><tr><th>接口 / ifIndex</th><th>状态</th><th>设备 description</th><th>接口自身 IP</th><th>${d.kind === 'router' ? 'ARP 邻居 IP / MAC' : '下挂 MAC / 关联 IP'}</th></tr></thead><tbody>${data.rows.map(i => `<tr><td><b>${esc(i.name)}</b><small class="mono">ifIndex ${i.ifIndex}</small></td><td>${badge(i.oper === 1 ? 'up' : i.oper === 2 ? 'down' : 'unknownPort')}<small>${i.oper === 1 ? 'Up' : i.oper === 2 ? 'Down' : '状态未知'}${d.uplinks.includes(i.ifIndex) ? ' · 上/下联' : ''}</small></td><td>${esc(i.description || '—')}<small>${esc(i.technical)}</small></td><td class="mono">${i.addresses.map(a => `${esc(a.ip)}${a.mask ? '<small>' + esc(a.mask) + '</small>' : ''}`).join('<br>') || '—'}</td><td>${d.kind === 'router' ? i.arp.map(a => `<span class="mono">${esc(a.ip)}</span><small class="mono">${esc(a.mac)}${a.static ? ' · 静态' : ''}</small>`).join('') || '—' : i.endpoints.map(f => `<code>${esc(f.mac)}</code><small>${f.ips.length ? f.ips.map(x => esc(x.ip)).join(' ; ') : 'IP 尚未关联'} · VLAN ${f.vlans.join(',') || '未知'}</small>`).join('') || '<span class="muted">未学习到 MAC</span>'}</td></tr>`).join('')}</tbody></table></div></div>`;
...
async function init() {
  ...
  setInterval(async () => {
    try { const next = await invoke('state'), wasActive = !!state.active; state = next; syncChrome();
      ...
      else if (!$('#modal').open && Date.now() - lastFreshnessRender > 60000) { lastFreshnessRender = Date.now(); await render(); }
